...
export default function Inventory() {
  ...
  const loadInventory = async () => {
    try {
      const [purchasedDataRaw, soldDataRaw] = await Promise.all([
        AsyncStorage.getItem('submittedData'),
        AsyncStorage.getItem('sellHistory'),
      ]);

      const purchasedData = purchasedDataRaw ? JSON.parse(purchasedDataRaw) : [];
      const soldData = soldDataRaw ? JSON.parse(soldDataRaw) : [];

      const soldMap = {};
      soldData.forEach((item) => {
        const key = `${item.category}-${item.name}`;
        if (!soldMap[key]) {
          soldMap[key] = { quantity: 0, price: 0 };
        }
        soldMap[key].quantity += parseFloat(item.quantity);
        soldMap[key].price += parseFloat(item.price);
      });

      const updatedData = purchasedData
        .map((item) => {
          const key = `${item.category}-${item.name}`;
          const sold = soldMap[key] || { quantity: 0, price: 0 };
          return {
            ...item,
            quantity: parseFloat(item.quantity) - sold.quantity,
            price: parseFloat(item.price) - sold.price,
          };
        })
  ...
  const inventoryData = submittedData.reduce((acc, item) => {
    ...
    if (!acc[category][item.name]) acc[category][item.name] = { quantity: 0, price: 0 };

    acc[category][item.name].quantity += parseFloat(item.quantity);
    acc[category][item.name].price += parseFloat(item.price);
    return acc;
  }, {});
